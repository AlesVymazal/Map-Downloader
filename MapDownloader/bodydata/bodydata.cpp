...
BodyData::BodyData(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::BodyData)
{
    ui->setupUi(this);
...
void BodyData::CreateDir()
{
    QDir dir;
    dir.mkdir(DirectoryName);
}

void BodyData::replyFinished(QNetworkReply* reply)
{
    if(reply->error())
    {
        qDebug() << "ERROR!";
...
bool BodyData::CheckInput(std::wstring Coordinate)
{
...
void BodyData::ToDecimal(double *Coordinate, std::wstring CoordinatesStr)
{
...
void BodyData::MapValues(double LatitudeNum, double LongitudeNum, Tile* TL, int Zoom)
{
    TL->zoom = Zoom;
